...
<div class="gsa-calendar">
  <iframe
    title="Berkeley–UCSF GSA calendar"
...
!!! note "One-time setup — replace `YOUR_CALENDAR_ID` above"
    ...
    `abc123@group.calendar.google.com`) and paste it in place of both
    `YOUR_CALENDAR_ID` above.

    The calendar must be **public**: on the same settings page, under *Access
    permissions*, tick **Make available to public**. If it isn't public, visitors
    see an empty box.
...
<p class="todo">Optional: also keep a short bullet list of the next few big dates
here, for people who skim. The embedded calendar is the source of truth.</p>
